...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        gui_path = os.path.join(path_to_main,'gui/eseem_main_window_insys.ui')
...
class Worker(QWidget):
    ...
    def exp_on(self, conn, p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p12, p13):
        ...
        if p13 == 0:
            ...
            pb.pulser_pulse(name = 'P3', channel = 'TRIGGER', start = PULSE_SIGNAL_START, length = adc_wind, delta_start = str(STEP) + ' ns') #, phase_list = ['+x']
        ...
        while self.command != 'exit':
            ...
            if p13 == 0:
                ...
                while j <= SCANS:
                    ...
                    for i in range(POINTS):
                        pb.pulser_update()
                        data[0], data[1] = pb.digitizer_get_curve( POINTS, PHASES, acq_cycle = ['+x'], integral = True )
                        general.plot_1d(p2, x_axis, ( data[0], data[1] ), xname = 'T',\
                                    xscale = 'ns', yname = 'Area', yscale = 'A.U.', label = p1, \
