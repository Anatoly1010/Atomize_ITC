...
| S5 Multi-Gaussian | **REVIEWED + LARGELY VERIFIED, 9 FIXED** (2026-08-06/07, two sessions). 11 reviewers → 53 raw findings → 10 triaged. Session 1: 23 skeptics, 23 CONFIRMED, 2 fixed. Session 2: `S5T-7` and `S5T-6` closed 2/2 and fixed, **`S5G-1` resolved by direct measurement — phenomenon and consequence real, mechanism refuted, half its fix shipped and half rejected**, and the long-r / weak-mode / mis-specified blind spot closed with three new catalogues. **7 more fixes, the whole set gated at exactly zero change over 156 + 144 synthetic + 28 real runs.** Outstanding: `S5T-8`/`S5T-9`/`S5T-10`, the DeerLab cross-check, and `S5G-2…S5G-5`. State banked at `~/deer_benchmark/s5_persist/round2/` | [REVIEW_S5_gauss.md](REVIEW_S5_gauss.md) |
...
## Session 2026-08-07 — S5 round 2: the blind spot closed, six fixes, all inert

Resumed from `~/deer_benchmark/s5_persist/RESUME.md` (out of session —
`resumeFromRunId` is same-session only and was long dead). Second compute box
`fel@172.16.16.1` brought into the loop for the catalogue work.

### First, a methodological trap that would have poisoned every number

`fel` reproduces `base.json` **closely but not bit-identically**: mean overlap
0.885022 vs 0.884158, correct-N 0.801 both, one run's N flipping (the
`easy/g4_uneven/0.02` pruning knife-edge already documented as a coin flip).
`deer.py` was md5-verified identical on both machines, so this is libm/BLAS.

**The cross-machine floor is Δoverlap ≈ 0.0009 — LARGER than real S5 effects**
(`noprune` is −0.0007). A variant run on `fel` paired against `base.json` would
have manufactured a significant-looking result out of nothing. Rule written into
`~/deer_benchmark/s5_gauss/README_CROSS_MACHINE.md`: pair against `fel_base.json`,
same box, always. The `S5T-6:mech` skeptic independently hit the same trap and
independently reached the same rule — which is the best evidence it is real.

### The blind spot is closed

Session 1's standing warning was that `base.json` is blind above ~5.3 nm and
below component weight 0.15, and that two "no measured consequence" conclusions
were wrong because of it. Three new catalogues remove the excuse — full tables in
[REVIEW_S5_gauss.md](REVIEW_S5_gauss.md):

| catalogue | runs | coverage |
|---|---|---|
| `bench_far.py` | 144 | **115 of 264 fitted components above 5.3 nm, 39 below the 0.10 spike gate** (`base.json`: none of either) |
| `floor_bind.py` | 90 | where the width floor actually binds |
| `bench_shapes.py` | 168 | mis-specified truths — the 21-shape catalogue |
| `real_bounds.py` | 28 real | bound occupancy on the ring test |

Four results, all new:

1. **The weak far mode is real and the engine finds it** — 0.10 weight at 6.2 nm
   hit 12/12, 0.08 at 6.0 nm 11/12, 0.05 at 5.5 nm 8/12. This matters because the
   S5-5 skeptics rejected a fix on the grounds it would destroy that mode; the
   mode had never actually been measured. **The hard case is weak-and-NEAR, not
   weak-and-far**: 0.06 weight at 3.9 nm beside a dominant 3.0 nm mode is hit only
   5/12 and, when missed, is reported with 8× its true weight. That inverts the
   intuition this review has been carrying.
2. **The width floor** biases σ by 0.164 nm when it binds and 0.002 nm when it
   does not — but in 24 of 39 binding cases the distance is not trace-supported
   anyway, where broadening is honest. So the actionable defect is **not** the
   constant 27 and **not** the grid: it is that a pinned bound is reported as a
   measurement.
3. **Mis-specification**: the *curve* survives (overlap 0.926 → 0.875, mean
   distance actually better), the *component table* does not — only **22 %** of
   components on non-Gaussian truths have a 95 % centre interval reaching any true
   mode, against 74 % on Gaussian ones; the hyperbola scores 0/22.
4. **Bound occupancy on real data**: 41 of 105 components sit on a box bound, and
   **all 28 traces** carry at least one. 15 are pinned at `r_max` = 8.000 nm
   holding 10–42 % of the weight, reported with centre bars up to **±84.9 nm**;
   38 of the 41 have a 95 % σ interval running below their own floor.
...
| `S5T-6` | mech (reach was already done) | **CONFIRMED** | fixed |
| `S5T-8` | mech CONFIRMED, **reach REFUTED** | **REFUTED** | behavioural half applied then **reverted**; only the cosmetic reword kept |
| `S5T-9` | mech + reach | **CONFIRMED 2/2** | fix applied, measured defective by the second lens, **reverted** — see below |
| `S5T-10` | mech + reach | **CONFIRMED 2/2** | fixed — the shipped smaller edit is the one both endorsed |

**The first refutation of the whole S5 stage** — 23 skeptics in session 1 all
confirmed, and that unanimity was flagged at the time as a warning about the gate
rather than a comfort. It was. Two of the four findings verified this session had
their fix rejected by their own second lens, one of them after I had already
applied it.

### `S5T-9` — confirmed 2/2, and the fix is NOT in the tree

Its `:mech` lens rejected the finding's own fix (V1: tagging by the blocked
wrapper leaves the headline broken, because the wrapper blocked by a
Background-tab edit *is* `do_deer`), rejected the obvious amendment (V2:
regresses an idle α edit), and specified V2B, which its QTest harness scored 5/5.
I applied V2B.

Its `:reach` lens then measured **a defect in V2B itself**: it tags
`_deer_pending` with the engine that asked and the drain then calls
`_rerun_shown_engine()`, **discarding the tag**. An explicit *Run Tikhonov* click
during a gauss fit is therefore converted into a Multi-Gaussian run — the exact
defect V2B's own comment claims to prevent, with the sign flipped. It also
refuted two things: the cost objection (`_deer_pending` is a **single slot**, so a
burst of 4 ticks yields **one** refit, not one per tick) and the finding's part-(c)
data-integrity escalation (`save_deer_all` reads `deer_result`, not the panel, so
a stale export is impossible — the exported header correctly says
"Tikhonov + NNLS").

**V2B has been reverted.** The landing version is specified and already measured
by that lens — tree + (1) the drain dispatching on the tag, (2) a
`_set_engine_panel()` that strikes a superseded engine's panel, (3) `clear_all`
clearing `gauss_info` — with f1/f2/f2b/f5 correct and **11/11** of the existing
`gui_tab/t*.py` behaviourally identical. It was not applied because this session
had already been burned once by shipping a GUI behaviour change I had not
verified myself, and the session ended before the harness could be re-run.
**Land it next session, then re-run `round2/t9c_fix.py`.**

Also from that lens, worth keeping: `deer_info` and `gauss_info` live on
different tabs of one `QTabWidget` and are **never visible together**, so the
"a careful user sees `(joint)` next to it" defence is false; what does contradict
a stale panel is the always-visible status line and the P(r) legend losing its
per-component overlay curves.

### `S5T-8` — the correction of record, and it goes against me

Its `:mech` lens confirmed the finding on a 252-run `bg_start` sweep and I applied
its fix. Its `:reach` lens then **refuted the finding and the fix together**:
every window measurement in the finding *and in the mech lens's own sweep* was
taken with **`Fit t0` forced OFF** by the test harness. At GUI defaults `Fit t0`
is **ON**, and then the background window is the zero-time fit's input — a
data-prep channel, not a seed. Measured at defaults, obeying the "start it later"
instruction moved the reported mean by up to **0.904 nm in 5 of 6** flagged cases
and **improved the error toward truth in all five**; with `Fit t0` off the same
quantity is 0.017 nm against 0.402 nm on.

The demotion I had shipped would therefore have printed *"moving it will not
shift the result"* beside a control that shifts the mean **4.1677 → 4.5693 nm on
a 5.00 nm truth**. **Reverted**; the flag is restored and the second tab note
requalified. Only the first note's solver-conditional reword survives, which both
lenses accept because it makes no behavioural promise.

**The process lesson is the finding here.** I applied a behavioural change on one
lens because it looked well-measured, and the second lens found the control that
invalidated it. The two-skeptic gate is not ceremony — it is the only thing that
caught this — and a measurement inherits every switch its harness silently set.
`S5T-9`'s fix is therefore held as provisional until its own second lens reports.

Both `S5T-7` skeptics independently built and byte-gated the *same* patch (21
files across 6 engine configurations, 0 failures) and both flagged the same trap:
`_csv_header_labels` takes the **last** `#` line as the column names, so text
appended after `col_header` silently breaks axis naming on reload. The reach lens
also promoted the band-column mislabel from footnote to headline — that band
ships at **default** settings and is 4.7× narrower than the lsq noise band.

`S5T-6`'s mech skeptic did the session's best work: it verified every link
(scipy 1.15.3 does raise on `lo == hi`), then found **a second injection point the
finding missed** — the fitted-centre re-floor loop rebuilds a degenerate box when
a centre *migrates* past c\* during the fit, even when every seed was legal, so a
seed-only guard would have been incomplete. Reachability is genuinely low:
**0/28 real traces trigger it** at the default grid, because `_reset_bg_window`
overwrites `r_max` with the trace-supported value on every load.

### Six fixes, and the convergence that produced the biggest one

Five separate findings' fix-gates (`S5T-3`, `S5T-2`, `S5-5`, `S5U-6`, `S5U-7`),
reviewed by different agents from different angles, all asked for the same
missing feature: **mark a parameter that came back on a bound**. That is fix A.

| | finding(s) | what |
|---|---|---|
| A | S5T-3, S5T-2, S5-5, S5U-6, S5U-7 | per-component `sigma_at_floor` / `sigma_at_ceiling` / `center_at_bound` / `bound_active` + the bounds actually imposed; panel and CSV print *(at width floor)* etc. in place of a ± bar |
| B | S5T-2 | component overlay rebuilt from `amplitude`/`lambda` — exact (1e-16) where it was **6.9–11.0 % of peak** off on truncated grids |
| C | S5T-2 | `mass` / `mass_fraction` added; **`weight` deliberately unchanged** |
| D | S5T-7, S5-4, S5T-4 | `solver =` in the CSV, mc band relabelled, `lambda_source='prep'` |
| E | S5-6, S5T-5, S5-5 | docstring / `deer.md` corrections |
| F | S5T-6 | strict `s_hi*0.999` cap + per-seed guard around `_solve` + returned `ic_failed` + a forced-N error that names the real cause |
| G | S5G-1 | `ic_railed` on both solvers + a GUI line beside the component count |
| H | S5T-8 | the first tab note's solver-conditional reword — **cosmetic half only**, the behavioural half was applied and reverted |
| J | S5T-10 | `' (auto)'` in place of the hard-coded `' (auto, AICc)'`, plus both defective tooltips |

Two fixes are deliberately NOT in the tree: `S5T-8`'s `bg_start_early` demotion
(refuted, above) and `S5T-9`'s V2B (measured defective by its second lens, above).

**Regression gate — the whole set together: 156-run base catalogue, 144-run
long-r catalogue, 28 real traces. Max abs Δoverlap 0.000e+00, max abs Δmean
0.000e+00, N differs on 0, pruning differs on 0, max abs Δcentre 0.000e+00.**
This is a reporting-layer change: it adds keys and corrects text, and moves no
fitted number anywhere it was measured. Offscreen GUI smoke run in
`~/deer_benchmark/s5_gauss/round2/smoke_final.py`.

A pleasing composition: `S5T-6`'s skeptic flagged a residual risk it could not
close — with the strict cap the offending component is pinned into
[0.999·s_hi, s_hi], a near-maximal-width Gaussian that `_has_spurious` cannot see
because it only tests *floor* width. Fix A's `sigma_at_ceiling` is exactly that
marker, so the two fixes cover each other.

### The DeerLab cross-check — ran at last, and only partly

It was found blocked behind a workflow phase whose agents were starved, so it was
pulled out and run standalone. Result: `s5_persist/round2/deerlab_xcheck.json`.

**What it establishes.** The conventions match, verified rather than assumed:
DeerLab's `bg_hom3d` prefactor measures out at **9.973857e-4 µs⁻¹ µM⁻¹** against
the shipped `K_BG_PER_UM` = 9.974e-4 (5 digits); λ is the same quantity on both
sides; the alias-clamped grid, zero time and `_echo_top` normalization were forced
identical; and evaluating DeerLab's dipolar model at `deer.simulate`'s parameters
reproduces it. On **4 single-Gaussian cases** the two engines are
indistinguishable — overlap 0.98211 vs 0.98212, mean |centre error| 0.00744 both,
paired t = −0.59 (p = 0.60).

**What it does NOT establish, and this is the larger half.** The real-trace
comparison produced **0 of 28 traces**, and the synthetic half ran **only N = 1**
— there is still no `dd_gauss2` / `dd_gauss3` comparison. So the multi-Gaussian
estimation this whole stage is about remains unchecked against an external
implementation; what is checked is the kernel, background and normalization
plumbing underneath it. **Re-queue for the next session.**

**A by-product worth more than it looks.** DeerLab's own default `par0` puts
*every* Gaussian mean at 3.5 nm — all N components starting on top of each other —
with a single start. On a clean 2-Gaussian synthetic that lands at **overlap 0.56
against truth** (RSS 0.0706 against a 0.0088 noise floor), and started *at* the
truth it recovers to RSS 0.0073: a search failure, not a model failure. That is an
independent corroboration of this stage's design-question-1 answer — multi-start
seeding is load-bearing, and the shipped engine's designed pair of seeds is doing
real work. (The comparison runs therefore used DeerLab's own documented
`multistart`, and its own box in a separate `dl_wide` arm.)

### `S5G-1` — resolved: phenomenon yes, mechanism no

Measured directly (`g1_check.py` → `g1_independent.json`, 28 real traces) because
both S5G-1 skeptics were starved on a box the other agents had saturated, and
this was the session's priority-1 claim.

**The mechanism is false.** The finding says the criterion has *no interior
minimum* and "would keep buying components well past 4 if allowed". Lift the cap:
`n_gauss_ic == max_gauss` on **25/28 at cap 4, 18/28 at cap 6, and 4/28 at
cap 8** — it turns over on **24 of 28 traces**, at N = 5–7. The criterion behaves
like a criterion; its minimum simply sits above the default cap.

**The phenomenon and the consequence hold.** At shipped settings N is the "N max"
spin box's value on 25/28 traces with nothing saying so, and that is not free:
between the selected N and N−1 the reported peak moves a median **0.131 nm** and
at worst **4.181 nm**, with P(r) overlap down to 0.647.

**The `n_eff` argument is an artefact of where it was measured.** L = 15–156 is
taken on the *forced N=1* residual, which still contains the components the extra
Gaussians exist to fit, so it measures signal as much as nuisance correlation.
The mech skeptic's own `lspace.json` shows L collapsing with N — **45.7 at N=1 →
5.2 at the selected N**, i.e. `n_eff` ≈ 91 against `npts` ≈ 456, a factor 5 rather
than 50 — and its `synth_neff.json` shows the `n_eff` and lag-1 patched criteria
returning **identical N to stock** on all five regimes. The remedy is inert.

Shipped: `ic_railed`. Rejected: recomputing the criterion on an effective sample
size or pre-whitening the residual.

Two independent synthetic controls support the narrowed reading — the criterion
rails on **8/144** of the long-r catalogue and **6/168** of the mis-specified
shapes against **25/28** real traces, so the railing is specific to real data's
systematic residual, at a cap that is too low for it.

### Fixes deliberately NOT shipped

Unchanged from session 1's fix-gate readings, and re-affirmed here: S5-5 option A
(measured regression, correct-N 0.843 → 0.731), S5T-1's `k_ratio` and `conc`
recomputes (both measured to break working detectors), S5T-4's "re-fit λ around
the mc optimum" (implemented by two skeptics, worse than what it replaces),
S5T-5's frequency-band engineering (the proposed low-cut amputates every dipolar
frequency past ~3.9 nm), S5-3's `k_collapsed` detector, S5-4's band suppression.

`S5T-1`'s full scope — the stale reliability keys including `deer_validate`'s
per-trial `flagged` vote — remains open. Both lenses want the namespace/label
route landed across four consumers at once; that is S6-sized, and shipping half
of it is worse than shipping none.
...
## Next session — finish S5, then S6
...
Still open from S5, in priority order:

1. **Port first** (both files are clean now), then land **`S5T-9`'s amended
   fix** — fully specified and already measured by its `:reach` lens (see above),
   just not applied. Re-run `~/deer_benchmark/s5_gauss/round2/t9c_fix.py` after,
   and regenerate its baseline: `t9c_base.json` was overwritten by a post-fix run
   and the `git HEAD` re-run crashed on Qt teardown.
2. **Finish the DeerLab cross-check** — `dd_gauss2`/`dd_gauss3` and the 28 real
   traces, neither of which produced data. Until then the multi-Gaussian
   estimator still has no external check; only the plumbing under it does.
3. `S5G-4` — the only genuinely open `verifynew`. `S5G-2` ≈ `S5-5` and
   `S5G-5` ≈ `S5T-2` are already verified 2/2 and acted on, and `S5G-3` was
   answered directly by `floor_bind.py`. `S5G-4` claims the stale-floor guard
   fails for a component migrating *inward*, which contradicts an item in the
   report's own *Cleared* table — one of the two is wrong.
4. The two **`S5G-1` skeptics never completed** (their workflow ended 0/2 after
   ~2 h starved on a saturated box). The finding is already resolved by direct
   measurement, so these are confirmatory only — re-run cheaply or drop.
5. Triage's cuts-for-cap, reasons in `triage_queue.json`: `callsites-1`,
   **`xengine-3`** (triage's own "strongest of the cuts"), `xengine-2`, `batch-1`,
   `me1-1`, `ci-1`, `status-1`, `robust-5`, `docs-7`.
6. **`S5T-1`'s full scope** — the stale reliability keys, including
   `deer_validate`'s per-trial `flagged` vote. Both lenses want the
   namespace/label route landed across four consumers at once; S6-sized, and
   shipping half is worse than shipping none.
7. A latent bug found in passing by `S5T-8`'s mech lens and **not fixed**:
   `deer_validate`'s `bg_cofit` test (the S5-1 fix from session 1) keys on
   `engine == 'gauss' and bg_engine != 'general'` and **ignores `method`** —
   harmless today only because the GUI skips validation for `mc`.

**Run the agents' heavy jobs on `fel`.** This session's local box hit load 54 on
4 cores because several agents ran multiprocessing pools with unpinned BLAS; the
two `S5G-1` skeptics were still crawling at ~15 % CPU after 2 h and never
finished. Tell agents to pin `OMP/OPENBLAS/MKL_NUM_THREADS=1` and to use the
remote box for anything catalogue-sized.

Carried into S5 by S4's own findings and STILL OPEN: `engine='gauss'` has the
**identical** `deer_validate` hole S4-1 fixed for Mellin — `n_gauss` is
re-selected per trial through the same `**kwargs`, so the validation band mixes
component counts. Separate from S5-1, which said the gauss band is degenerate.

**The port is five sessions deep and is a straight file copy** (verified
2026-08-07): `deer.py` and `deer_analysis.py` are byte-identical across plain /
NIOCH / NIOCH_Q / Cryomech and equal to ITC's pre-S5 state, so there is no
hand-merge — copy ITC's two files out. `deer_analysis.py` exists only in ITC /
NIOCH / NIOCH_Q. Port the two together (`band_degenerate`, the bound flags and
`ic_railed` are produced in one and consumed in the other), and run
`~/atomize_sync/sync_check.py` first.

**Both files are portable as they stand.** The two changes that were not fully
verified — `S5T-8`'s `bg_start_early` demotion and `S5T-9`'s V2B — have been
reverted, so the tree carries only fixes whose gate is closed. Port at the start
of the next session, before landing anything new on top.
