...
`tune.rep_rate` reuses `Worker.dig_on` with GUI `l_mode=0`, which calls the driver with `live_mode=1`. Field and τ remain fixed across a logarithmic rate grid. Each ordinary, nonempty `digitizer_get_curve` result provides an echo integral; software loop iterations and phase cycles are not observations. The stability policy accepts three consecutive returned curves when `(max - min) / mean <= 0.05`, then stores their mean complex signal and requests the next rate. The result is consumed as returned; an epoch (rate-index) filter drops frames left over from a previous rate, and any remaining mixed-rate buffer content is handled by the stability check rather than further packet-tag filtering. `points` increases the window; `scans` requests disjoint stable groups, with a combined 5% check. `max_wait` limits each rate, including buffer arrival.
